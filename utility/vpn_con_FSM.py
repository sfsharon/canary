...
def ErrorTransition (fsm):
    logging.error('ErrorTransition FSM : That does not compute.')
...
def EnterPassword (fsm):
    logging.info('EnterPassword : Sending password.')
    fsm.memory['pexpect_child'].sendline('123456')
    # sys.exit(2)

def ErrorKillPexpectProc(fsm) :
    logging.info('ErrorKillPexpectProc : Restarting VPN process')
    child = fsm.memory['pexpect_child']
    child.close()
    logging.info(f"{child.exitstatus}, {child.signalstatus}")
...
def main() :
    ...
    f.add_transition         (TWO_FACTOR_REQUEST_SYMBOL, 'INIT', ErrorKillPexpectProc, 'INIT')
